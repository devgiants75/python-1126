# matplotlib_pokemon.py

# Counter 모듈을 사용하여 데이터의 빈도수를 계산
# : 이터러블(iterable, '리스트, 튜플, 문자열 등') 데이터의 각 요소가 얼마나 자주 있는지 계산
# > 변수 = Counter(이터러블 데이터)
...
# 포켓몬 타입 데이터를 분리하여 모든 타입 리스트 생성
all_types = [] # 모든 타입을 담을 리스트 초기화

for _, _, types in pokemon_data: # pokemon_data의 각 항목에서 타입 정보를 추출
    all_types.extend(types.split('/')) # 슬래시('/')로 나뉜 타입을 리스트에 추가
...
plt.pie(counts, labels=types, autopct='%1.1f%%', startangle=90, colors=plt.cm.Paired.colors)
plt.title('타입별 포켓몬 비율', fontsize=16)
